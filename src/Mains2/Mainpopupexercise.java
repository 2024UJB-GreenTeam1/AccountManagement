...
public class Mainpopupexercise {
	...
	public static void main(String[] args) {
		...
		TextField mykcaltext = new TextField(7);

		mykcaltext.setSize(70, 25);
		mykcaltext.setLocation(165, 675);
		mykcaltext.setFont(font1);
		...
		exercisereference3.setSize(110, 40);
		exercisereference3.setLocation(45, 670);
		exercisereference3.setFont(font1);
		...
		todayexercisebutton.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            // myweighttext 필드에서 입력된 값을 읽어옵니다.
	            String weight = myweighttext.getText();
	            // Mainpopup 클래스의 todayweightText 필드에 값을 설정합니다.
	            Mainpopup.todayweightText.setText(weight);
	        }
	    });
		...
		exercisesccbutton.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e) {
		        try {
		            String input = exercisetext.getText();
		            Matcher matcher = Pattern.compile("\\d+").matcher(input);
		            int sum = 0;
		            while (matcher.find()) {
		                sum += Double.parseDouble(matcher.group());
		            }
		            mykcaltext.setText(String.valueOf(sum));
		        } catch (NumberFormatException nfe) {
		            mykcaltext.setText("error");
		        }
		    }
		});
